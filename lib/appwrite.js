...
export const deleteVideo = async (video, thumbnail, videoId) => {
    try {

        const videoURL = new URL(video);
        const thumbnailURL = new URL(thumbnail);

        // Split the pathname and get the video file ID
        const videoPathSegments = videoURL.pathname.split('/');
        const videoFileId = videoPathSegments[videoPathSegments.indexOf('files') + 1];

        // Split the pathname and get the thumbnail file ID
        const thumbnailPathSegments = thumbnailURL.pathname.split('/');
        const thumbnailFileId = thumbnailPathSegments[thumbnailPathSegments.indexOf('files') + 1];
        ...
        console.log('Appwrite deleteVideo() error :', error)
        throw new Error(error);
    }
}
